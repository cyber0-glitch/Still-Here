const STORAGE_KEY = 'stillhere_mvp_state_v5';
...
function loadState() {
  ...
  try {
    ...
    return { ...defaultState(), ...parsed, users, blockedPhrases: parsed.blockedPhrases?.length ? parsed.blockedPhrases : DEFAULT_BLOCKED_PHRASES };
  } catch {
    return defaultState();
  }
}
...
function ensureView() {
  ...
  if (me && !needsOnboarding) renderAll();
}
...
function renderAdmin() {
  ...
  dom.queueCount.textContent = String(pending.length);
  ...
  const pendingMemories = state.users
    .filter((user) => user.memorial?.isMemorial)
...
dom.authForm.addEventListener('submit', (e) => {
  ...
  if (!user) {
    user = { id: crypto.randomUUID(), name, email, prompts: {}, onboarded: false, verificationStatus: 'basic', allowCaregiverView: false, caregivers: [], memorial: { preference: 'undecided', message: '', designatedName: '', designatedContact: '', isMemorial: false, memories: [] } };
...
dom.messageForm.addEventListener('submit', (e) => {
  ...
  if (!convo || !text || convo.memorialLocked) return;
  const flagged = autoFlagIfScam(text, state.currentUserId, 'message');
  convo.messages.push({ id: crypto.randomUUID(), senderId: state.currentUserId, text, createdAt: Date.now() });
  const otherId = convo.participantIds.find((id) => id !== state.currentUserId);
  notify(otherId, `${currentUser().name}: ${text}`);
...
dom.profileSaveBtn.addEventListener('click', () => {
  ...
  notify(me.id, `Profile updated: ${verificationBadge(me.verificationStatus)} verification, caregiver view ${me.allowCaregiverView ? 'on' : 'off'}.`);
  saveState();
  renderAll();
});
